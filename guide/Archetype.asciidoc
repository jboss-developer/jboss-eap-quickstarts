...
You can create a project from the archetype using Red Hat JBoss Developer Studio, or Eclipse with JBoss Tools. First, open up _Red Hat Central_, if it isn't already open. Hit _Cmd-3_ (Mac) or _Ctrl-3_ (Windows, Linux) and type _Red Hat Central_:

image:gfx/Eclipse_JBoss_Central_1.png[]

You will now be shown _Red Hat Central_, an excellent place to find about all things JBoss!
