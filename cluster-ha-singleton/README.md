...
4. This deploys `service/target/cluster-ha-singleton-service.jar` to the running instance of the first server.
...
6. This deploys `service/target/cluster-ha-singleton-service.jar` to the running instance of the additional server.
...
7. To verify the application deployed to each server instance, check the server logs. The first instance should have the following message:
   ...
   The other servers will have the message:
        ...
        WFLYSRV0010: Deployed "cluster-ha-singleton-service.jar" (runtime-name : "cluster-ha-singleton-service.jar")
...
_NOTE_: If you have not yet configured the JBoss EAP 7.1 runtime in JBoss Developer Studio, choose `Window`--> `Preferences` --> `Runtime Environment` and click `Add` to configure the following server instances.
...
3. Configure the first server instance in JBoss Developer Studio.
   ...
   * In the `Add and Remove` dialog, add the `cluster-ha-singleton-service` to the `Configured` list and click `Finished`.
...
5. To deploy the cluster-ha-singleton service to `EAP7-Server1`, right-click on the `cluster-ha-singleton-service` project, choose `Run As` --> `Run on Server`, choose `EAP7-Server1` and click `Finish`. Note the messages in the `EAP7-Server1` server console indicate it is the singleton provider of the service.
...
6. To deploy the cluster-ha-singleton service to `EAP7-Server2`, right-click on the `cluster-ha-singleton-service` project, choose `Run As` --> `Run on Server`, choose `EAP7-Server2` and click `Finish`. Note that `EAP7-Server1` is still the singleton provider of the service. This message is in the `EAP7-Server2` console.
